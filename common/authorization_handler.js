...
module.exports.authenticate = (req, res, next) => {
  ...
  if (token == null) {
    response_handler.set_failure_response(res, 401, 'Unauthorized access', req);
  }

  try {
    jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, (error, user) => {
      if (error) {
        response_handler.set_failure_response(res, 403, 'Forebidden access', req);
      }
      req.user = user;
      next();
    });
...
module.exports.is_authorized = async (privilege_name, req, res) => {
  ...
  var has_privilege = await has_user_privilege(req.user.user_id, privilege_name);
  if (!has_privilege) {
      response_handler.set_failure_response(res, 403, 'Forebidden access', req);
      return false;
  }
